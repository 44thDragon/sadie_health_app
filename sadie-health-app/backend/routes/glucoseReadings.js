...
// Define the route for submitting a backdated glucose reading
  ...
  if (req.body.food) {
      ...
      if (foodError) {
        ...
        dbConnection.query(glucoseQuery, glucoseValues, (glucoseError, glucoseResults) => {
          if (glucoseError) {
            ...
            console.log('Glucose reading stored successfully');
            res.status(201).json({ message: 'Backdated glucose reading and food data stored successfully' });
          }
        });
      }
    });
    ...
    dbConnection.query(glucoseQuery, glucoseValues, (glucoseError, glucoseResults) => {
      if (glucoseError) {
        ...
        console.log('Glucose reading stored successfully');
        res.status(201).json({ message: 'Backdated glucose reading stored successfully' });
      }
    });
  }
});
